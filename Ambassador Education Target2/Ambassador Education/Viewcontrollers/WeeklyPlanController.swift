...
class WeeklyPlanController: UIViewController,MXSegmentedPagerDelegate,MXSegmentedPagerDataSource,TaykonProtocol {
    ...
    func setDatesOnPicker(){
        ...
        self.toDateLabel.text = "To : \(getThe5thDayFromSelectedDate(date: today, value: 4))"
    }
                ...
                if self.weeklyPlan != nil{
                    if let div = self.weeklyPlan?.divisions{
                        if div.count > 0{
                            ...
                            self.getWeeklyPlanDetails(fromDate: "", toDate: "", isSearch: 1, div: self.divId)
                        }
                    }
                }
            }
    ...
    func getWeeklyPlanDetails(fromDate:String,toDate:String,isSearch : Int,div : String){
        startLoadingAnimation()
            ...
            for each in completeListDetails!{
                if each.value is NSArray{
                    if let value = each.value as? NSArray{
                        if value.count > 0{
                            self.tiltesnew.append((each.key as! String).uppercased())
    ...
    func setDownLoadUrl(result : NSDictionary,type:String){
        if let attachmentLink = result["AttachmentLink"] as? String{
            if type == "P"{
                self.loadPDFAndShare(url: attachmentLink, formatString: "downloadPdf.pdf")
            }
            else if type == "W"{
                self.loadPDFAndShare(url: attachmentLink, formatString: "downloadDoc.doc")
                    ...
                    if each.contains("-") || each.contains("/"){
                        ...
                        if let dt = toDateLabel.text {
                            ...
                            if  let dates = dt.components(separatedBy: "-") as? [String]{
                                ...
                                let nextDate = getThe5thDayFromSelectedDate(date: date as NSDate, value: -4)
                                self.fromDateLabel.text = "From : " + nextDate//+ convertToSlash(date: nextDate)
                                
                                self.getWeeklyPlanDetails(fromDate: nextDate, toDate: newdate, isSearch: 0, div: divId)
                            }
                    ...
                    if each.contains("-") || each.contains("/"){
                        ...
                        if let dt = fromDateLabel.text {
                            ...
                            if  let dates = dt.components(separatedBy: "-") as? [String]{
                                ...
                                let date =  changetoDiffFormatInDate(value:dt,fromFormat:format ,toFormat:"yyyy-MM-dd hh:mm:ss")
                                let nextDate = getThe5thDayFromSelectedDate(date: date as NSDate, value: +4)
                                 self.toDateLabel.text = "To : " + nextDate
                                    self.getWeeklyPlanDetails(fromDate: newdate, toDate: nextDate, isSearch: 0, div: divId)
                                }
                            }
    ...
    func setPagerProporties(){
        ...
        let attributeFontSaySomething : [String : Any] = [NSForegroundColorAttributeName : UIColor.white,NSFontAttributeName : UIFont.boldSystemFont(ofSize: 14.0)]
        //
    ...
    func loadPDFAndShare(url: String,formatString: String){
        ...
        videoDownload.startDownloadingUrls(urlToDowload:[urlString!],type:"")
   ...
   func segmentedPager(_ segmentedPager: MXSegmentedPager, viewForPageAt index: Int) -> UIView {
    ...
    if index < tiltesnew.count{
        var titless = self.tiltesnew[index]
    ...
    func showPopUpView(){
        ...
        popUpViewVc = BIZPopupViewController(contentViewController: popvc, contentSize: CGSize(width:300,height: CGFloat(400)))
        self.present(popUpViewVc!, animated: true, completion: nil)
        
    }
    
    func getBackToParentView(value: Any?, titleValue: String?) {
        if let values = value as? (String,String,Int,String){
            
            let formatedStart = values.0.replacingOccurrences(of: "-", with: "/")
            let formatedEnd = values.1.replacingOccurrences(of: "-", with: "/")
            divId = values.3
            
            self.getWeeklyPlanDetails(fromDate:formatedStart, toDate: formatedEnd,isSearch: values.2, div: divId )
        }
