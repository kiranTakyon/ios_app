...
class WeeklyPlanFilterController: UIViewController,UITextFieldDelegate{
    ...
    func setSubjectDropDown(){
        ...
        if dataSources.count > 0{
            ...
            self.subjectID = filterSubIdWrtName(item: dataSources[0], array: self.subjects!)
            subjectDropDown?.selectionAction = { (index: Int, item: String) in
                print("Selected item: \(item) at index: \(index)")
                self.subjectID = self.filterSubIdWrtName(item: item, array: self.subjects!)
    ...
    func filterSubIdWrtName(item : String,array :[subjectlist]) -> String{
        if array.count > 0{
            for each in array{
    ...
    @IBAction func searchAction(_ sender: Any) {
        if selectClassField.text != "" && selectSubject.text != "" && startingDateField.text != "" && endingDateField.text != ""{
            isSearch = 0
            let fullTime = (startTimeString,endTimeString,isSearch,divId )
