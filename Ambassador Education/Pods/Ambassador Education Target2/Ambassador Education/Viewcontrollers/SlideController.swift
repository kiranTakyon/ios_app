...
class SlideController: UITableViewController,TaykonProtocol {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toGradeBook"{
            ...
            let vc = destinationVC.childViewControllers[0] as? GradeViewController
            if gradeBookLink != "" {
    ...
    func getMenuDetails() {
        ...
        for menu in unsortedItems{
            ...
            if menu.label != ""{
                if menu.parentid == "0"{
                    if menu.hashKey.safeValue == "T0008" || menu.hashKey.safeValue == "T0004"{
        ...
        if sectionHeaderItems.count > 0{
            ...
            for eachSection in sectionHeaderItems{
                ...
                if eachSection.label == "Finance" || eachSection.label ==  "المالية"{
                ...
                else if eachSection.label == "Academics" || eachSection.label == "الخطط والواجبات المنزلية"{
                    if isFirst{
                        zeroParentIds.insert(eachSection, at: index)
                    }
                    else {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            ...
            if cell.titleLabel.text! == "Locator & Navigator" || cell.titleLabel.text == "تحديد المواقع والملاحة" {
                self.openBuzzApp()
            } else {
    ...
    func setTableCellColor(indexPath:IndexPath,cell:slidemenuCell){
        ...
        if cell.titleLabel.text == "Academics" ||  cell.titleLabel.text == "Finance" ||  cell.titleLabel.text == "الخطط والواجبات المنزلية" ||  cell.titleLabel.text == "المالية" {
            cell.isSection = true
        }else{
    ...
    func openBuzzApp(){
        ...
        if !UIApplication.shared.canOpenURL(appURL){
            ...
            if #available(iOS 10.0, *) {
                UIApplication.shared.open(ns!, options: dict, completionHandler: nil)
            }
            else{
                UIApplication.shared.open(ns!, options: dict, completionHandler: nil)
            }
        ...
        if !titles.contains(acadamicsInnerArray[0]) {
            ...
            for element in acadamicsInnerArray{
                ...
                if titles.contains(element){
                    ...
                    guard  let index = titles.index(of: element) else {return}
...
class slidemenuCell : UITableViewCell{
    ...
    func setSectionHeader(){
        ...
        if isSection{
            self.titleLabelContainerLeading.priority = 999
            self.titleToImageView.priority = 500
            self.titleLabel.textColor = UIColor.lightGray
            self.topSeperationView.isHidden = false
        }else{
            self.titleLabelContainerLeading.priority = 500
            self.titleToImageView.priority = 999
