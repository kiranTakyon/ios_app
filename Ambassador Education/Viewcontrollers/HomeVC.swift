...
class HomeVC: UIViewController,UITableViewDataSource, UITableViewDelegate,SWRevealViewControllerDelegate {
    ...
    func addObserverToNotification(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(openNotification(notification:)), name: Notification.Name(rawValue: "onMessageReceived"), object: nil)
    }
        ...
        APIHelper.sharedInstance.apiCallHandler(url, requestType: MethodType.POST, requestString: "", requestParameters: dictionary) { (result) in
            DispatchQueue.main.async {
                ...
                if result["StatusCode"] as? Int == 1 {
                    let row = index.row
                    if row < self.notificationList.count {
                        print("Removing row at: \(row)")
                        self.notificationList.remove(at: row)
                        self.alertTable.deleteRows(at: [index], with: .right)
                        self.navigateToSpecificPage(typeValue: selectedAlertType)
                    } else {
                        print("❗️Index out of range: \(row), list count: \(self.notificationList.count)")
                    }
                }
