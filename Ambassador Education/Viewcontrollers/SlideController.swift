...
//            vc.text = notificationObject.title
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // Reset link and title
        gradeBookLink = ""
        var titleValue = ""

        // 1️⃣ Extract item and log its hashKey (for non-header rows)
        if indexPath.row != 0 {
            let item = menuListItems[indexPath.row - 1]
            print("🔑 Tapped hashKey: \(item.hashKey)")

            // If there's a link, capture it and log
        ...
        // 2️⃣ Handle header row
        if indexPath.row == 0 {
            return
        }

        // 3️⃣ Safely cast the cell and read its hashKey
        guard let cell = tableView.cellForRow(at: indexPath) as? slidemenuCell else {
            print("⚠️ Could not cast cell at \(indexPath) to slidemenuCell")
            return
        }
        let hashKey = cell.hashKey
        print("🔑 Cell.hashKey: \(hashKey)")
        ...
        // 5️⃣ Handle LMS by link content
        if let linkString = gradeBookLink, linkString.lowercased().contains("lms") {
            if let url = URL(string: linkString), UIApplication.shared.canOpenURL(url) {
                UIApplication.shared.open(url, options: [:], completionHandler: nil)
            } else {
                print("⚠️ Invalid LMS URL: \(linkString)")
            }
            return
        }
        ...
        // 6️⃣ Finance cases
        switch hashKey {
        case APIUrls().paymentDetails:   finance = 1
        case APIUrls().feeDetails:       finance = 2
        case APIUrls().feeSummary:       finance = 3
        case APIUrls().transfeeSummary:  finance = 4
        case APIUrls().absenceReport:    finance = 5
        default:
            break
        }

        // 7️⃣ Other segues
        let segueVal = getSegueIdentifier(title: hashKey)
        sibling = false
        selectedHash = hashKey

        print("   → segue identifier: \(segueVal.isEmpty ? "none" : segueVal)")
        if !segueVal.isEmpty {
            performSegue(withIdentifier: segueVal, sender: titleValue.isEmpty ? nil : titleValue)
        }
    }

    // Your safe segue lookup remains unchanged
    func getSegueIdentifier(title: String) -> String {
        return menuListDetails[title]?[1] ?? ""
    }
