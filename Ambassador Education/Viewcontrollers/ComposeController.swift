...
class ComposeController: UIViewController, RichEditorToolbarDelegate, TaykonProtocol, UITextFieldDelegate, UIDocumentPickerDelegate, KSTokenViewDelegate {
    ...
    func checkFieldsEmpty(isForDraft: Bool = false) -> (Bool, String, Int) {
        if !isForDraft {
            if groupView.text == "" {
...
extension ComposeController: TopHeaderDelegate {
    ...
    func sendEmailOrDraft(isForDraft: Bool = false) {
        ...
        // Pass isForDraft flag to checkFieldsEmpty
        if checkFieldsEmpty(isForDraft: isForDraft).0 != true {
            if attachmentTypes.count > 0 {
                for each in attachmentTypes {
                    ftpUrls.append(each.attachmentItem)
                }
            ...
            SweetAlert().showAlert(kAppName, subTitle: checkFieldsEmpty(isForDraft: isForDraft).1, style: AlertStyle.error)
        }
    }
