...
class ComposeController: UIViewController,RichEditorToolbarDelegate,TaykonProtocol,UITextFieldDelegate,UIDocumentMenuDelegate,UIDocumentPickerDelegate ,KSTokenViewDelegate {
    ...
    func setTableViewProporties(){
        ...
        self.attachmentTb.rowHeight = UITableViewAutomaticDimension
            ...
            if let statusCode = result["StatusCode"] as? Int{
                DispatchQueue.main.async {
                    ...
                    if statusCode == 1{
                        ...
                        self.view.endEditing(true)
                        self.stopLoadingAnimation()
                        SweetAlert().showAlert("Success", subTitle: "Mail Sent", style: .success, buttonTitle: alertOk, action: { (index) in
                            if index {
    ...
    func checkForStudAndParent(fileUrls: [String]){
        ...
        guard let user = UserDefaultsManager.manager.getUserType() as? String else {return}
        ...
        if token != nil{
            ...
            if deletedId != ""{
                ...
                if groupIds.count > 0{
                    if groupIds.contains(Int(deletedId).safeValueOfInt){
                        var index = groupIds.index(of: (Int(deletedId)).safeValueOfInt)
            ...
            if seletedGroups.contains("\(token)"){
                ...
                if fullArr.count > 0{
                    for each in fullArr{
                        if each == "\(token)"{
                            let index = fullArr.index(of: each)
