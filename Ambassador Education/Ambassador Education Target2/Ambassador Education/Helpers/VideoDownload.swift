...
class VideoDownload {
    ...
    func saveSrtData(srt:NSData, destination:NSURL) -> Bool {
        if srt.write(to: destination as URL, atomically: true) {
            ...
            checkAndStopDownload(fileUrl:destination)
            print(Date())
            return true
        }
        return false
    ...
    func startDownloadingUrls(urlToDowload:[String],type: String){
     ...
     //   delegate?.loadingStarted()
        ...
        for url in srtUrls  {
            ...
            let urlLastComponenet = (url?.lastPathComponent)!
            ...
            if FileManager().fileExists(atPath: destinationUrl.path) {
                ...
                checkAndStopDownload(fileUrl:url!)
            } else {
                print(Date())
                ...
                getSrtDataFromUrl(srtUrl: originalSubtitleURLStr) { data in
                    DispatchQueue.main.async {
                        ...
                        if self.saveSrtData(srt: data!, destination:destinationUrl as NSURL) {
    ...
    func checkAndStopDownload(fileUrl:NSURL) {
        
        var folderName = getFolderName(path:(fileUrl.deletingPathExtension?.lastPathComponent)!)

        let urlLastComponenet = fileUrl.lastPathComponent
        var str = ""
